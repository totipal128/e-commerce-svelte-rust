<script>
    ...
    async function deleteData() {
        loading = true
        try {
            await invoke("consumer_delete", {
                id: id
            })
            showToast("Berhasil menghapus data pelanggan", "success");
        } catch (err) {
            console.log("err", err)
			...
			<div class="text-center mt-5">
                ...
                {:else}
                    <h3 class="mb-6 text-body">
                        Apakah Anda Yakin Ingin Menghapus Data Pelanggan <strong>{data.name || ''}</strong> ?
                    </h3>
                {/if}
