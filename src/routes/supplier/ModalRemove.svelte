<script>
    ...
    async function deleteData() {
        loading = true
        try {
            await invoke("supplier_delete", {
                id: id
            })
            showToast("Berhasil menghapus data pemasok", "success");
        } catch (err) {
            console.log("err", err)
			...
			<div class="text-center mt-5">
                ...
                {:else}
                    <h3 class="mb-6 text-body">
                        Apakah Anda Yakin Ingin Menghapus Data Pemasok (Supplier) <strong>{data.name || ''}</strong> ?
                    </h3>
                {/if}
