<script>
    ...
    async function fetchData() {
        loading = true
        try {
            const result = await invoke("supplier_by_id", {
                id: id
            })
            if (result.data) {
            ...
            console.log("err", err)
            showToast("Gagal mengambil data pemasok: " + err, "error");
        } finally {
            loading = false;
        }
    }
    ...
    async function updateData() {
        loading = true
        try {
            const result = await invoke("supplier_update", {
                data: supplierData
            })
            showToast("Berhasil memperbarui data pemasok", "success");
            closeModal()
        } catch (err) {
            console.log("err", err)
