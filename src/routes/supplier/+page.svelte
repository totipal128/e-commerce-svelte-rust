<script>
    ...
    async function fetchData() {
        loading = true
        try {
            const result = await invoke("supplier_list", {
                filter: {
                    search: searchDb,
                }
            })
	...
	{:else}
		<div class="flex justify-between">
			<div class="ml-5 mt-3 flex">
				<button onclick={handlerAdd} type="button" class="text-white bg-brand box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">Tambah Data Supplier</button>
			</div>
			...
			<form class="mt-3 mr-5 w-100" onsubmit={(e)=>(search("enter"))}>
				...
				<div class="relative">
					...
					<input
							...
							placeholder="Pencarian supplier..."
		...
		<Table headers={headers} data={data} on:remove={handlerRemove} on:update={handlerUpdate} on:detail={handlerUpdate} />
	{/if}
</div>
