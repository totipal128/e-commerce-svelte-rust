<script>
  ...
  async function fetchData() {
    loading = true;
    try {
      const result = await invoke("unit_barang_by_id", {
        id: id,
      });
      if (result.data) {
      ...
      console.log("err", err);
      showToast("Gagal mengambil data satuan: " + err, "error");
    } finally {
      loading = false;
    }
  }
  ...
  async function updateData() {
    loading = true;
    try {
      const result = await invoke("unit_barang_update", {
        data: unitData,
      });
      showToast("Berhasil memperbarui data satuan", "success");
      closeModal();
    } catch (err) {
      console.log("err", err);
        ...
        <form class="relative mt-5" on:submit|preventDefault={save}>
          <div class="grid grid-cols-2 gap-4 place-items-stretch">
            ...
            <div class="col-span-2">
              ...
              >
              <textarea
                bind:value={unitData.description}
                rows="2"
                class="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
