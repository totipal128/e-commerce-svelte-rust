<script>
    ...
    async function fetchData() {
        loading = true
        try {
            const result = await invoke("jenis_barang_list")
            data = result.results;
            ...
            console.log("err", err)
            showToast("Gagal mengambil data kategori barang: " + err, "error");
        } finally {
            loading = false;
        }
    }
	...
	{:else}
		<div class="flex justify-between">
			<div class="ml-5 mt-3 flex">
				<button onclick={handlerAdd} type="button" class="text-white bg-brand box-border border border-transparent hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">Tambah Kategori</button>
			</div>
