use crate::app::master_data::model::items::{ItemPrice, Items, ItemsCreate, ItemsDetail};
...
pub async fn create_sale(mut data: SaleDetail) -> Result<Sale, String> {
    ...
    if data.created_by.is_some() {
        qs = qs.insert_i32("created_by", data.created_by.unwrap())
    }
...
pub async fn update_sale(data: SaleDetail) -> Result<Sale, String> {
    ...
    if let Some(item) = &data.items {
        for (k, i) in item.iter().enumerate() {
            if i.id.unwrap() < 1 {
                continue;
            }
        }
